...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--num-to-remove', type=float, default=0.9, help='Percentage of empty slices to remove')
    parser.add_argument('-ts', '--tumoral-slices-only', action='store_true', help='Load only the tumor region from the segmentation mask')
    ...
    if args.remove_empty:
        ...
        num_to_set_false = int(empty_slices_num * (1 - args.num_to_remove))
        print('Removing empty slices (keeping {}% | {} out of {})...'.format(int((1 - args.num_to_remove) * 100), num_to_set_false, empty_slices_num))
    ...
    if args.tumoral_slices_only:
        tumoral_slices_map = data[:, -1].mean(axis=(1, 2)) > data[:, -1].min()
        print('Keeping tumoral slices only ({} out of {})...'.format(tumoral_slices_map.sum(), data.shape[0]))
        data = data[tumoral_slices_map]
        if args.slice_idx:
            slice_levels = slice_levels[tumoral_slices_map]
    ...
    dataset['images'] = data
    if args.slice_idx: 
        dataset['slice_idx'] = slice_levels
 
    # saving the dataset as a npy file
    np.save('{}/first_stage_dataset_{}x{}{}.npy'.format(
        args.save_path, args.target_shape[0], args.target_shape[1], '_tumoral' if args.tumoral_slices_only else ''
