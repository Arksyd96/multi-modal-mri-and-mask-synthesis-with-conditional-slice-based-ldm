...
if __name__ == "__main__":
    ...
    parser.add_argument('-bg', '--background', action='store_true', help='Background brain shape')
    ...
    for idx, instance in enumerate(tqdm(instances, position=0, leave=True, desc="Processing patients")):
        ...
        if 'seg' in args.modalities:
            if args.binarize:
                volumes['seg'] = (volumes['seg'] > 0).astype(np.float32)
                volumes['seg'] = 2 * volumes['seg'].astype(np.float32) - 1 # normalize to [-1, 1]
            else: # multilabel
                volumes['seg'][volumes['seg'] == 4] = 3 # merge label 4 with 3 [unexisting label 3]
                if args.background:
                    volumes['seg'] = np.where(
                        volumes['seg'] == 0, 
                        (volumes['flair'] > 0).astype(np.float32), 
                        volumes['seg'] + 1
                    )
                volumes['seg'] = 2 / (4 if args.background else 3) * volumes['seg'].astype(np.float32) - 1 # normalize to [-1, 1]

        # stacking, normalizing and transposing ...
        volumes = np.stack([volumes[m] for m in args.modalities], axis=0)

        # patient-wise normalization of images
        for idx_m in range(args.modalities.__len__() - 1):
